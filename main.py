...
class MorePersonLike(Star):
    ...
    async def initialize(self):
        ...
        # 获取子项配置
        self.poke_enabled = self.poke_config.get("is_enable", False)
        self.poke_prompt = self.poke_config.get("poke_prompt", "请以可爱的语气，生成一句被人戳了的拒绝回应，表现出些许娇嗔或不满，但整体保持可爱风格。")
        self.pokeback_probability = self.poke_config.get("pokeback_probability", 0.2)
        ...
        # 获取子项配置
        self.active_message_enabled = self.active_message_config.get("is_enable", False)
        self.active_message_prompt = self.active_message_config.get("active_message_prompt", "长时间没有人搭理你，因此请你以可爱的语气，生成一条消息寻求大家陪你聊天或玩耍，表现出些许娇嗔或不满，但整体保持可爱风格。")
        self.time_interval = self.active_message_config.get("time_interval", 3600)
    ...
    async def active_message(self, event: AstrMessageEvent):
        ...
        if time_elapsed >= self.time_interval:
            ...
            try:
                ...
                # 使用配置中的提示语
                prompt = self.active_message_prompt
